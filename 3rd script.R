...
dir.create(here("results3"), showWarnings = FALSE)
...
write_csv(analysis_sales, here("results3", "clean_analysis_sales_high_competition.csv"))
write_csv(analysis_sales_full, here("results3", "clean_analysis_sales_full_high_competition.csv"))
...
datasummary_skim(
  desc_continuous,
  output = here("results3", "table_01_descriptive_statistics.docx")
)
...
print(doc, target = here("results3", "table_02_distribution_tables.docx"))
...
ggsave(
  here("results3", "fig_01_sales_by_high_competition.png"),
  fig_sales_high_comp,
  width = 7,
  height = 4,
  dpi = 300
...
ggsave(
  here("results3", "fig_02_box_sales_by_high_competition.png"),
...
ggsave(
  here("results3", "fig_03_productivity_by_high_competition.png"),
...
modelsummary(
  ...
  output = here("results3", "table_03_main_sales_high_competition.docx"),
  stars = TRUE,
...
modelsummary(
  ...
  output = here("results3", "table_04_alternative_outcomes_high_competition.docx"),
  stars = TRUE,
  coef_omit = "sector_strata|country_id",
  gof_omit = "IC|Log|RMSE",
...
modelsummary(
  ...
  output = here("results3", "table_05_robustness_checks_high_competition.docx"),
  stars = TRUE,
...
modelsummary(
  ...
  output = here("results3", "table_06_heterogeneity_results_high_competition.docx"),
  stars = TRUE,
  coef_omit = "sector_strata|country_id",
  gof_omit = "IC|Log|RMSE",
...
capture.output(
  vif_results,
  file = here("results3", "diagnostic_vif_results.txt")
)
...
ggsave(
  here("results3", "fig_04_residual_plot_sales_model.png"),
  fig_residuals,
  width = 7,
  height = 4,
  dpi = 300
...
writeLines(
  summary_text,
  here("results3", "econometric_summary_notes.txt")
)
